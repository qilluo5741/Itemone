...
<html>
  ...
  <body>
<button class="btn btn-primary btn-lg btn-info" data-toggle="modal" data-target="#myModal" id="addBtn" >添加角色</button>
  <hr/>
	...
	$(function(){
		...
		$.confirm=function(obj){
			...
			if(val==""){
				...
				//执行修改
				$.ajax({
					url:'web/permiss/updateRole.do',
					data:{"roleId":id,"roleName":val},
					type:"post",
					success:function(res){
						if(res=="0"){
							alert("这个角色已经存在了哟！不能重复修改哈");
						}else if(res=="1"){
							var $del_=$(obj).parent().parent().parent().parent(".roleName");
							$del_.html(val);
							$del_.next().children(".update_").removeAttr("disabled");
		...
		$(".delete_").click(function(){
			if(confirm("你确定要删除吗？")){
				...
				$.ajax({
				url:'web/permiss/deleteRole.do',
				data:{"roleId":$(this).attr("id")},
				type:"post",
				success:function(res){
					if(res=="0"){
						alert("该角色关联了用户哟！不能删除！");
					}else if(res=="1"){
						//得到当前这一行对象删除
						$rem.remove();
					}else{
		...
		$("#addSbm").click(function(){
			...
			if(roleName==""){
				...
				//添加菜单
				$.ajax({
					url:'web/permiss/addRole.do',
					data:{"roleName":roleName},
					type:"post",
					success:function(res){
						if(res=="0"){
							alert("\""+roleName+"\" 这个角色已经存在了哟！不能重复添加哈！");
						}else if(res=="1"){
							//刷新页面
		...
		$(".manager_").bind("click",function(){
			window.location.href="web/permiss/role_menuManager.do?roleId="+$(this).attr("id");
		})
	});
</script>
<!-- 模态框（Modal） -->
